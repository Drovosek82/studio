        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase font-bold text-accent opacity-70 tracking-[0.2em]">Intelligence Monitoring</p>
            </div>
          </div>
        ...
        <Tabs defaultValue="aggregated" className="w-full">
          <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 md:w-[800px] mb-8 bg-secondary/40 h-12">
            ...
            <TabsTrigger value="intelligence" className="gap-2">
              <BrainCircuit className="h-4 w-4" /> База знань ШІ
            </TabsTrigger>
            <TabsTrigger value="settings" className="gap-2">
              <Settings className="h-4 w-4" /> Опції
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="aggregated" className="space-y-6">
            {aggregated ? (
              <>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                  <Card className="glass-card border-none p-4">
                    <p className="text-muted-foreground text-[10px] font-bold uppercase">Середня напруга</p>
                    <h3 className="text-2xl font-bold text-accent">{aggregated.totalVoltage.toFixed(2)} V</h3>
                  </Card>
                  <Card className="glass-card border-none p-4">
                    <p className="text-muted-foreground text-[10px] font-bold uppercase">Сумарний струм</p>
                    <h3 className="text-2xl font-bold text-accent">{aggregated.totalCurrent.toFixed(1)} A</h3>
                  </Card>
                  <Card className="glass-card border-none p-4">
                    <p className="text-muted-foreground text-[10px] font-bold uppercase">Потужність</p>
                    <h3 className="text-2xl font-bold text-accent">{(aggregated.totalPower / 1000).toFixed(2)} kW</h3>
                  </Card>
                  <Card className="glass-card border-none p-4">
                    <p className="text-muted-foreground text-[10px] font-bold uppercase">Середній SoC</p>
              ...
              <div className="text-center py-20 bg-secondary/10 rounded-2xl border border-dashed border-border">
                ...
                <h3 className="text-lg font-bold">Очікування даних</h3>
                <p className="text-xs text-muted-foreground mt-2">Підключіть свій перший пристрій для початку моніторингу</p>
              </div>
            )}
          </TabsContent>
                ...
                <Card className="glass-card border-none">
                  <CardHeader>
                    <CardTitle className="text-lg flex items-center gap-2">
                      <Server className="h-5 w-5 text-accent" /> Локальний Хаб
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="flex gap-2">
                      <input 
                        className="flex-1 bg-secondary/30 border border-border/50 rounded-md px-3 h-10 text-sm focus:outline-none focus:ring-1 focus:ring-accent"
                        placeholder="IP (напр. 192.168.1.50)"
                        value={localHubIp}
                        onChange={(e) => setLocalHubIp(e.target.value)}
                      />
                      <Button variant="outline" onClick={() => setLocalHubIp('')}>Скинути</Button>
                    </div>
                  </CardContent>
                </Card>
                <FirmwareWizard />
